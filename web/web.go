	...
	engine.NoRoute(func(c *gin.Context) {
		...
		if middleware.IsLocalWhitelistHost(c.Request.Host) {
			if trimmedBase != base_url && path == trimmedBase {
				c.Redirect(http.StatusMovedPermanently, base_url)
				return
			}
			if strings.HasPrefix(path, base_url) {
		...
		// Any path NOT under the correct base_url gets fake 504
		if !strings.HasPrefix(path, base_url) {
			fake504Handler(c)
			return
		}

		// Under correct base_url - normal SPA handling
		if path != base_url+"login" && !api.IsLogin(c) {
			fake504Handler(c)
			return
		}
		if path == base_url+"login" && api.IsLogin(c) {
...
func (s *Server) Start() (err error) {
	...
	if len(materials) > 0 {
		...
		if loadErr != nil || len(certs) == 0 {
			s.clearTLSState()
			if loadErr != nil {
				logger.Warning("failed to load TLS certificate set, falling back to HTTP:", loadErr)
			}
		...
		s.clearTLSState()
		if materialErr != nil {
			logger.Warning("failed to resolve TLS material, falling back to HTTP:", materialErr)
		}
		listener = network.NewAutoHttpListener(listener)
		logger.Info("web server run http on", listener.Addr())
	}
...
func (s *Server) getTLSCertificate(hello *tls.ClientHelloInfo) (*tls.Certificate, error) {
	...
	if !ok || len(materials) == 0 {
		return nil, fmt.Errorf("web tls certificate not loaded")
	}
...
func (s *Server) ReloadTLSCertificateMaterials(materials []*service.PanelTLSMaterial) (string, error) {
	...
	if !active {
		return "", fmt.Errorf("web listener is not running in https mode")
	}
...
// ReloadTLSCertificate reloads certificate/key files for the running HTTPS web listener.
...
// TLSState returns whether web is currently serving HTTPS and its in-use certificate metadata.
...
func (s *Server) releaseTLSSelection(selection service.PanelCertificateBalanceSelection) {
	...
	if err := s.balanceService.Release(selection); err != nil {
		logger.Warning("web tls certificate selection release failed:", err)
	}
}
