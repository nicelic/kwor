...
func parseMihomoCoreVersionWindowPagination(c *gin.Context) (string, int, int) {
...
func parseMihomoCoreIntervalHours(raw string) (int, error) {
...
type mihomoCoreDownloadStopRequest struct {
	ID *string `json:"id" form:"id"`
}

func parseMihomoCoreDownloadStopRequest(c *gin.Context) (string, error) {
	req := mihomoCoreDownloadStopRequest{}
...
func (a *ApiService) GetMihomoCoreDownloadProgress(c *gin.Context) {
	jsonObj(c, service.GetMihomoManagedCoreDownloadProgress(c.Query("id")), nil)
}

func (a *ApiService) StopMihomoCoreDownload(c *gin.Context) {
	id, err := parseMihomoCoreDownloadStopRequest(c)
	if err != nil {
		jsonMsg(c, "", err)
		return
	}
	progress, err := service.StopMihomoManagedCoreDownload(id)
	jsonObj(c, progress, err)
}

func parseMihomoCoreVersionWindowQuery(c *gin.Context) mihomoCoreVersionRequest {
	channel, offset, limit := parseMihomoCoreVersionWindowPagination(c)
...
func parseMihomoCoreUpdateSettingsRequest(c *gin.Context) (mihomoCoreUpdateSettingsRequest, error) {
	...
	case "auto_update":
		...
		return request, nil
	case "interval", "":
		intervalHours, err := parseMihomoCoreIntervalHours(c.Request.FormValue("interval"))
		if err != nil {
