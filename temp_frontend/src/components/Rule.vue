<template>
  <v-card style="background-color: inherit;">
    <v-row>
      ...
      <v-col cols="12" sm="6" md="4" v-if="!isMihomoNamespace && optionIPver">
        <v-select
          hide-details
          :label="$t('rule.ipVer')"
          :items="[4,6]"
      ...
      <v-col cols="12" sm="6" v-if="!isMihomoNamespace && optionProtocol">
        <v-select
    ...
    <v-row v-if="isMihomoNamespace && optionProcess">
      <v-col cols="12" sm="6" md="4">
        <v-select
          hide-details
          :items="processKeys"
          @update:model-value="updateProcessOption($event)"
      ...
      <v-col cols="12" sm="6" v-if="rule.process_name != undefined">
        ...
        label="Process Name"
        hide-details
        v-model="process_name"></v-text-field>
      </v-col>
      <v-col cols="12" sm="6" v-if="rule.process_path != undefined">
        <v-text-field
        label="Process Path"
        hide-details
        v-model="process_path"></v-text-field>
      </v-col>
      ...
      <v-menu v-model="menu" :close-on-content-click="false" location="start">
        ...
        <v-card>
          <v-list>
            ...
            <v-list-item>
              <v-switch v-model="optionClient" color="primary" :label="authUserLabel" hide-details></v-switch>
            </v-list-item>
...
<script lang="ts">
export default {
  props: ['rule', 'namespace', 'clients', 'inTags', 'rsTags', 'deleteable'],
  data() {
    return {
      menu: false,
  ...
  methods: {
    sanitizeRule() {
      ...
      ;[
        ...
        'protocol',
        'user',
      ].forEach((key: string) => delete this.$props.rule[key])
    },
  ...
  computed: {
    ...
    authUserItems(): string[] {
      return this.isMihomoNamespace ? [] : (this.$props.clients ?? [])
    },
    authUserLabel(): string {
      return this.isMihomoNamespace ? 'IN-USER' : this.$t('pages.clients')
    },
    optionIPver: {
    ...
    optionProcess: {
      get() { return this.processKeys.some((key: string) => this.$props.rule[key] != undefined) },
      set(v:boolean) {
        if (v) {
          this.$props.rule.process_name = []
        } else {
          this.processKeys.forEach((key: string) => delete this.$props.rule[key])
        }
        this.processOption = 'process_name'
      }
    },
    optionUserID: {
      get() { return this.$props.rule.user_id != undefined },
      set(v:boolean) { this.$props.rule.user_id = v ? [] : undefined }
    },
    domain: {
