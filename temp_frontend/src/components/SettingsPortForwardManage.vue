<template>
  <section class="pf-page">
          ...
          <v-card-text class="pf-hero__content">
            ...
            <div class="pf-hero__chips">
              ...
              <v-chip
                v-if="hasLoaded"
                size="small"
                :color="overview.kernelIPv4Forward ? 'success' : 'warning'"
                variant="flat"
                class="pf-hero-chip">
                <v-icon start size="14">{{ overview.kernelIPv4Forward ? 'mdi-check-circle-outline' : 'mdi-alert-circle-outline' }}</v-icon>
                {{ t('kernelIPv4') }}: {{ overview.kernelIPv4Forward ? t('forwardOn') : t('forwardOff') }}
              </v-chip>
              <v-chip
                v-if="hasLoaded"
                size="small"
                :color="overview.kernelIPv6Forward ? 'success' : 'warning'"
                variant="flat"
                class="pf-hero-chip">
                <v-icon start size="14">{{ overview.kernelIPv6Forward ? 'mdi-check-circle-outline' : 'mdi-alert-circle-outline' }}</v-icon>
                {{ t('kernelIPv6') }}: {{ overview.kernelIPv6Forward ? t('forwardOn') : t('forwardOff') }}
              </v-chip>
            </div>
            ...
            <v-row class="mt-2">
              <v-col cols="12" sm="6" md="3">
                <div class="pf-metric">
                  <div class="text-caption pf-muted-label">{{ t('enabledRules') }}</div>
                  <div class="text-h5 mt-1">{{ overview.enabledCount }}</div>
                </div>
              </v-col>
              <v-col cols="12" sm="6" md="3">
                <div class="pf-metric">
                  <div class="text-caption pf-muted-label">{{ t('limitedRules') }}</div>
                  <div class="text-h5 mt-1">{{ overview.limitedCount }}</div>
                </div>
              </v-col>
              <v-col cols="12" sm="6" md="3">
                <div class="pf-metric">
                  <div class="text-caption pf-muted-label">{{ t('totalUpload') }} / {{ t('totalDownload') }}</div>
                  <div class="text-subtitle-1 mt-1 font-weight-bold d-flex align-center flex-wrap ga-1">
                    <span class="text-success d-inline-flex align-center ga-1">
                      <v-icon size="14">mdi-arrow-up</v-icon>{{ formatTrafficGB(overview.totalUp) }}
                    </span>
                    <span class="text-medium-emphasis">/</span>
                    <span class="text-info d-inline-flex align-center ga-1">
                      <v-icon size="14">mdi-arrow-down</v-icon>{{ formatTrafficGB(overview.totalDown) }}
                    </span>
                  </div>
                </div>
              </v-col>
              <v-col cols="12" sm="6" md="3">
                <div class="pf-metric">
                  <div class="text-caption pf-muted-label">{{ t('totalTraffic') }}</div>
                  <div class="text-h5 mt-1">{{ formatTrafficGB(overview.totalTraffic) }}</div>
                </div>
              </v-col>
            </v-row>
          </v-card-text>
    ...
    <v-alert v-if="loadError || overview.error || runtimeError" type="warning" variant="tonal" density="comfortable" class="my-4">
      {{ loadError || overview.error || runtimeError }}
    </v-alert>
    <v-alert v-if="hasLoaded && !overview.available" type="info" variant="tonal" density="comfortable" class="my-4">
      {{ t('unavailableHint') }}
    </v-alert>

    <!-- 规则管理主体表格：直接进入首屏 -->
    <v-card rounded="xl" variant="outlined" class="pf-table-card mt-4">
      <v-card-title class="pf-table-card__toolbar">
        <div class="d-flex align-center ga-2 flex-wrap">
          <v-icon color="primary" size="22">mdi-format-list-bulleted</v-icon>
          <span class="text-subtitle-1 font-weight-bold">{{ t('tableTitle') }}</span>
          <v-chip size="x-small" color="primary" variant="tonal" class="ml-1 font-weight-medium">
            {{ filteredRules.length }} / {{ overview.rules.length }}
          </v-chip>
          <span class="text-caption text-medium-emphasis ml-2 d-none d-sm-inline">{{ t('tableSubtitle') }}</span>
        </div>
      </v-card-title>
      <v-divider />
      <v-card-text>
        <v-row class="mb-2">
          <v-col cols="12" md="5"><v-text-field v-model="searchText" :label="t('searchLabel')" prepend-inner-icon="mdi-magnify" clearable hide-details density="comfortable" variant="outlined" /></v-col>
          <v-col cols="12" md="3"><v-select v-model="familyFilter" :items="familyFilterItems" :label="t('familyFilter')" hide-details density="comfortable" variant="outlined" /></v-col>
          <v-col cols="12" md="4"><v-select v-model="protocolFilter" :items="protocolFilterItems" :label="t('protocolFilter')" hide-details density="comfortable" variant="outlined" /></v-col>
        </v-row>
        ...
        <v-data-table v-if="!smAndDown" :headers="headers" :items="filteredRules" item-value="id" fixed-header class="rounded-lg pf-table" hide-no-data>
          ...
          <template #item.target="{ item }">
            <div class="py-2 font-weight-medium d-flex align-center ga-1">
              <v-icon size="14" color="primary" class="d-none d-md-inline">mdi-arrow-right</v-icon>
              <span>{{ targetDisplayLabel(item.targetIP, item.targetPort) }}</span>
            </div>
          </template>
          ...
          <template #item.limit="{ item }">
            <div class="py-2">
              ...
              <div v-if="item.limitStatus === 'degraded' && item.limitWarning" class="text-caption text-warning">{{ t('limitDegraded') }}：{{ item.limitWarning }}</div>
              <div v-else class="text-caption text-medium-emphasis">{{ t('leftPortOnly') }}</div>
            </div>
          </template>
          ...
          <v-card v-for="item in filteredRules" :key="item.id" variant="outlined" rounded="lg" class="pf-mobile-rule">
            <v-card-text>
              <div class="d-flex align-start justify-space-between ga-2">
                <div>
                  <div class="font-weight-medium text-subtitle-1">{{ item.name || t('ruleFallback') }}</div>
              ...
              <!-- 移动端流向与连接卡片 -->
              <div class="pf-mobile-route mt-3 pa-3 rounded-lg">
                <div class="d-flex align-center justify-space-between flex-wrap ga-2">
                  <div class="d-flex align-center ga-1 font-weight-bold">
                    <span class="text-primary">{{ item.localPortSpec || '-' }}</span>
                    <v-icon size="14" color="medium-emphasis">mdi-arrow-right</v-icon>
                    <span>{{ targetDisplayLabel(item.targetIP, item.targetPort) }}</span>
                  </div>
                  <div class="d-flex align-center ga-1">
                    <v-chip size="x-small" variant="flat" class="pf-protocol-chip">{{ protocolLabel(item.protocol) }}</v-chip>
                    <v-chip size="x-small" color="info" variant="outlined">{{ familyLabel(item.family) }}</v-chip>
                  </div>
                </div>
                <div class="text-caption text-medium-emphasis mt-1">{{ localModeLabel(item.localPortMode) }}</div>
              </div>

              <div class="pf-mobile-grid mt-3">
              ...
              <div class="d-flex align-center justify-end ga-1 mt-3 pt-2 border-t">
                <v-switch density="compact" color="success" hide-details inset :model-value="item.enabled" :loading="rowBusyId === item.id" :disabled="actionsDisabled" :aria-label="t('toggleRule', { name: item.name || t('ruleFallback') })" @update:modelValue="value => toggleRule(item, Boolean(value))" />
                <v-btn icon="mdi-restart" size="small" variant="text" color="warning" :aria-label="t('resetRuleTraffic', { name: item.name || t('ruleFallback') })" :title="t('resetTraffic')" :disabled="actionsDisabled" @click="resetRuleTraffic(item)" />
                <v-btn icon="mdi-pencil" size="small" variant="text" color="primary" :aria-label="t('edit')" :title="t('edit')" :disabled="actionsDisabled" @click="openRuleDialog(item)" />
                <v-btn icon="mdi-delete" size="small" variant="text" color="error" :aria-label="t('delete')" :title="t('delete')" :disabled="actionsDisabled" @click="removeRule(item)" />
        ...
        <div v-if="filteredRules.length === 0" class="pf-empty">
          ...
          <div class="text-subtitle-2 mt-2">{{ t('emptyText') }}</div>
        </div>
      </v-card-text>
    </v-card>

    <!-- 运行态与底层系统诊断：优雅折叠面板，绝不霸占首屏 -->
    <v-expansion-panels v-model="runtimePanelOpen" class="mt-4 pf-runtime-panels">
      <v-expansion-panel rounded="xl" elevation="0" class="pf-runtime-panel">
        <v-expansion-panel-title class="pf-runtime-panel__title py-3 px-4">
          <div class="d-flex align-center justify-space-between flex-wrap ga-2 w-100 pr-2">
            <div class="d-flex align-center ga-2">
              <v-icon color="info" size="20">mdi-tune-variant</v-icon>
              <span class="text-subtitle-1 font-weight-medium">{{ t('runtimeTitle') }}</span>
              <v-chip size="x-small" :color="hasLoaded && overview.supported && overview.ready ? 'success' : 'warning'" variant="flat">
                {{ !hasLoaded ? t('unknown') : overview.ready ? t('ready') : t('no') }}
              </v-chip>
            </div>
            <div class="text-caption text-medium-emphasis d-none d-sm-flex align-center ga-3">
              <span>{{ t('nftVersionLabel') }}: {{ overview.nftVersion || '-' }}</span>
              <span>·</span>
              <span>{{ compatibilityModeLabel }}</span>
              <span>·</span>
              <span>{{ t('totalTraffic') }}: {{ formatTrafficGB(overview.totalTraffic) }}</span>
            </div>
          </div>
        </v-expansion-panel-title>
        <v-expansion-panel-text class="pf-runtime-panel__body pt-2">
          ...
          <div class="d-flex align-center justify-space-between flex-wrap ga-3 mt-3 pt-3 border-t">
            <div class="text-caption text-medium-emphasis d-flex align-center ga-2">
              ...
              <span>{{ t('runtimeHint') }}</span>
            </div>
            <v-btn variant="tonal" color="warning" size="small" prepend-icon="mdi-restart" :loading="overviewResetBusy" :disabled="actionsDisabled" @click="resetOverviewTraffic">
              {{ t('resetOverviewTraffic') }}
            </v-btn>
          </div>
...
const runtimePanelOpen = ref<number | undefined>(undefined)
const {
...
.pf-hero__content, .pf-table-card { position: relative; }
...
.pf-side__row { padding: 10px 14px; display: flex; justify-content: space-between; margin-bottom: 8px; gap: 12px; }
...
.pf-mobile-route { background: rgba(15, 23, 42, 0.38); border: 1px solid rgba(148, 163, 184, 0.12); }
...
/* 运行态折叠面板卡片样式 */
.pf-runtime-panels { border-radius: 18px; }
.pf-runtime-panel { border: 1px solid rgba(148, 163, 184, 0.18) !important; background: rgba(15, 23, 42, 0.28) !important; }
.pf-runtime-panel__title { background: transparent !important; }
