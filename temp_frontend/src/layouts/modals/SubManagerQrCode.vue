<template>
  <v-dialog v-model="dialogVisible" transition="dialog-bottom-transition" width="calc(100vw - 32px)" max-width="400">
    <v-card class="rounded-lg" id="sm-qrcode-modal" :loading="loading || subscriptionUriLoading">
      <v-card-title>
        <v-row>
          <v-col>QrCode</v-col>
          <v-spacer></v-spacer>
      ...
      <v-divider></v-divider>
      <v-card-text style="overflow-y: auto; padding: 0" v-if="tag">
...
export default {
  ...
  methods: {
    ...
    copyToClipboard(txt: string) {
      ...
      hiddenButton.className = 'sm-clipboard-btn'
      document.body.appendChild(hiddenButton)

      const clipboard = new Clipboard('.sm-clipboard-btn', {
        text: () => txt,
        container: document.getElementById('sm-qrcode-modal') ?? undefined
      })
  ...
  computed: {
    ...
    subJsonUrl() {
      ...
      return Data().subURI + "q/sm?tag=" + encodeURIComponent(this.tag) + "&format=json"
    },
    subClashUrl() {
      if (!this.subscriptionUriReady) return ''
      return Data().subURI + "q/sm?tag=" + encodeURIComponent(this.tag) + "&format=clash"
    },
    singboxUrl() {
      if (!this.subJsonUrl) return ''
      return "sing-box://import-remote-profile?url=" + encodeURIComponent(this.subJsonUrl) + "#" + encodeURIComponent(this.tag)
    },
    size() {
