    ...
    <v-card class="rounded-lg">
      ...
      <v-card-text style="padding: 0 16px;">
        ...
        <v-row>
          ...
          <v-col cols="12" sm="6" md="4" v-if="logical && !isMihomoNamespace">
            <v-combobox
...
export default {
  ...
  methods: {
    ...
    updateData() {
      if (this.$props.index != -1) {
        const rawData = JSON.parse(this.$props.data)
        const newData = this.normalizeRule(rawData)
        if (newData?.type) {
        ...
        if (this.isMihomoNamespace) {
          ...
          this.ruleData.mode = 'and'
          this.ruleData.invert = false
          this.ruleData.action = this.ruleData.action === 'reject' ? 'reject' : 'route'
        }
        this.title = 'edit'
      }
      else {
    ...
    saveChanges() {
      ...
      const normalized = this.normalizeRule(newRule)
      if (normalized != null) {
  ...
  computed: {
    ...
    availableActions(): { title: string; value: string }[] {
      return this.isMihomoNamespace
        ? this.actions.filter((item: { value: string }) => item.value === 'route' || item.value === 'reject')
        : this.actions
    },
    logical: {
