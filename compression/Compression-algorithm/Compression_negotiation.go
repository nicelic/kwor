...
// upstream HTTP service. Both the list order and explicit descending q-values
// carry the same fixed priority, so servers that honor q and servers that only
// use list order receive the same zstd > s2 > snappy > br > deflate > gzip
// signal. These q-values are generated by this project, not interpreted from
// the client request; local selection itself treats every q>0 as equal.
func UpstreamAcceptEncoding() string {
	priority := fixedPriorityOrder()
	items := make([]string, 0, len(priority))
	for index, algorithm := range priority {
		quality := 1.0 - float64(index)*0.001
