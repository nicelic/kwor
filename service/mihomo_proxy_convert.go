...
type mihomoProxyConversionResult struct {
	...
	UnsupportedTag map[string]string
	ValidationErrs []string
}

func convertMihomoOutboundsToClash(rawOutbounds []map[string]interface{}) *mihomoProxyConversionResult {
	result := &mihomoProxyConversionResult{
		Proxies:        make([]map[string]interface{}, 0, len(rawOutbounds)),
		ProxyGroups:    make([]map[string]interface{}, 0, len(rawOutbounds)),
		SupportedTags:  map[string]struct{}{"DIRECT": {}, "REJECT": {}, "REJECT-DROP": {}},
		DirectTags:     map[string]struct{}{},
		OutboundTypes:  make(map[string]string, len(rawOutbounds)),
		UnsupportedTag: make(map[string]string),
		ValidationErrs: []string{},
	}
	...
	for _, outbound := range rawOutbounds {
		...
		result.OutboundTypes[tag] = outType
		outboundByTag[tag] = outbound
		if outType == "direct" {
			result.DirectTags[tag] = struct{}{}
		}
	}
	...
	for _, outbound := range rawOutbounds {
		...
		switch outType {
		case "selector":
			group := buildMihomoSelectGroup(tag, outbound)
			result.ProxyGroups = append(result.ProxyGroups, group)
			result.SupportedTags[tag] = struct{}{}
		case "urltest":
			group := buildMihomoURLTestGroup(tag, outbound)
			result.ProxyGroups = append(result.ProxyGroups, group)
			result.SupportedTags[tag] = struct{}{}
		case "direct":
			continue
		case "tor":
			result.UnsupportedTag[tag] = outType
			continue
		}
	}

	for _, obMap := range rawOutbounds {
		...
		outType = strings.TrimSpace(outType)
		if outType == "selector" || outType == "urltest" || outType == "direct" || outType == "tor" {
			continue
		}

		if proxy, ok := buildStoredMihomoRawClashProxy(tag, outType, obMap); ok {
			...
			if detourTag != "" {
				if detourOutbound, exists := outboundByTag[detourTag]; exists {
					if detourType, _ := detourOutbound["type"].(string); detourType == "shadowtls" {
						if proxy, ok := buildMihomoShadowTLSSSProxy(obMap, detourOutbound); ok {
							result.Proxies = append(result.Proxies, proxy)
							result.SupportedTags[tag] = struct{}{}
							continue
						}
		...
		if outType == "shadowtls" {
			ssOutbound, shadowTLSOutbound := util.BuildShadowTLSRuntimeOutboundPairMap(obMap, true)
			if ssOutbound != nil && shadowTLSOutbound != nil {
				if proxy, ok := buildMihomoShadowTLSSSProxy(ssOutbound, shadowTLSOutbound); ok {
					result.Proxies = append(result.Proxies, proxy)
					result.SupportedTags[tag] = struct{}{}
					continue
				}
		...
		proxy, ok := buildMihomoProxy(tag, outType, obMap)
		if !ok {
			result.UnsupportedTag[tag] = outType
			continue
		}
	...
	for _, group := range result.ProxyGroups {
		members := normalizeMihomoGroupMembers(group["proxies"], result)
		if len(members) == 0 {
			name, _ := group["name"].(string)
			result.ValidationErrs = append(result.ValidationErrs, fmt.Sprintf("proxy group %q has no valid supported members", strings.TrimSpace(name)))
		}
		group["proxies"] = members
	}

	normalizeMihomoProxyDialerTargets(result)

	return result
}

// normalizeMihomoProxyDialerTargets validates the target after every proxy
// and runtime proxy-group has been collected. A panel subscription group is
// intentionally absent from SupportedTags, so it cannot be emitted as a
// dangling dialer-proxy reference in server.yaml.
func normalizeMihomoProxyDialerTargets(result *mihomoProxyConversionResult) {
	if result == nil {
		return
	}
	...
	for _, proxy := range result.Proxies {
		...
		rawTarget := strings.TrimSpace(firstString(proxy["dialer-proxy"]))
		if rawTarget == "" {
			delete(proxy, "dialer-proxy")
			continue
		}

		target, ok := normalizeMihomoTarget(rawTarget, result)
		if !ok || target == "REJECT" || target == "REJECT-DROP" {
			name := strings.TrimSpace(firstString(proxy["name"]))
			if name == "" {
				name = "unnamed"
			}
			result.ValidationErrs = append(result.ValidationErrs,
				fmt.Sprintf("proxy %q dialer-proxy references unavailable Mihomo proxy or proxy group %q", name, rawTarget))
			continue
		}
...
func buildStoredMihomoRawClashProxy(tag string, outType string, outbound map[string]interface{}) (map[string]interface{}, bool) {
	if outbound == nil {
		return nil, false
	}
	...
	if rawProxy, ok := extractStoredMihomoImportedClashProxy(outbound); ok {
		...
		if generated {
			mergeImportedMihomoRawClashProxy(proxy, rawProxy)
		} else {
			proxy = cloneMap(rawProxy)
		}
		if proxy == nil {
			return nil, false
		}
		...
		delete(proxy, "tag")
		delete(proxy, mihomoImportedClashProxyKey)
		sanitizeUnsupportedMihomoClashProxyFields(proxy)
		return proxy, true
	}
	...
	// Keep manual sing-box style outbounds on the legacy conversion path.
	if _, hasServerPort := outbound["server_port"]; hasServerPort {
		return nil, false
	}

	_, hasPort := outbound["port"]
	portRange := strings.TrimSpace(firstString(outbound["port-range"]))
	if !hasPort && portRange == "" {
		return nil, false
	}

	proxy := cloneMap(outbound)
	if proxy == nil {
		return nil, false
	}
...
func extractStoredMihomoImportedClashProxy(outbound map[string]interface{}) (map[string]interface{}, bool) {
	if outbound == nil {
		return nil, false
	}
...
func buildStoredMihomoImportedClashProxy(tag string, outType string, outbound map[string]interface{}) (map[string]interface{}, bool) {
	cloned := cloneMap(outbound)
	if cloned == nil {
		return nil, false
	}

	delete(cloned, mihomoImportedClashProxyKey)
	delete(cloned, "name")
	delete(cloned, "port")
	delete(cloned, "port-range")
	cloned["tag"] = tag
	if strings.TrimSpace(outType) != "" {
		cloned["type"] = outType
	}
	...
	delete(result.Proxies[0], mihomoImportedClashProxyKey)
	return result.Proxies[0], true
}

func mergeImportedMihomoRawClashProxy(dst map[string]interface{}, src map[string]interface{}) {
	outType := strings.ToLower(strings.TrimSpace(firstString(dst["type"])))
	mergeImportedMihomoRawClashProxyByType(dst, src, outType)
}

func mergeImportedMihomoRawClashProxyByType(dst map[string]interface{}, src map[string]interface{}, outType string) {
	if dst == nil || src == nil {
		return
	}

	for key, value := range src {
		if !shouldPreserveImportedMihomoRawKey(key, outType) || isEmptyMergedFieldValue(value) {
			continue
		}
		...
		if srcIsMap && dstIsMap && dstMap != nil {
			mergeImportedMihomoRawClashProxyByType(dstMap, srcMap, outType)
		}
	}
}

func shouldPreserveImportedMihomoRawKey(key string, outType string) bool {
	switch key {
	case "",
		"id",
		"name",
		...
		"quic",
		"health-check":
		if outType == "trusttunnel" && (key == "udp" || key == "health-check") {
			return true
		}
		return false
	default:
...
func buildMihomoSelectGroup(tag string, outbound map[string]interface{}) map[string]interface{} {
	group := map[string]interface{}{
		"name": tag,
		"type": "select",
	}

	members := toStringSlice(outbound["outbounds"])
	defaultMember := strings.TrimSpace(firstString(outbound["default"]))
	if defaultMember != "" {
		members = movePreferredMemberToFront(members, defaultMember)
	}
	group["proxies"] = members
...
func buildMihomoURLTestGroup(tag string, outbound map[string]interface{}) map[string]interface{} {
	group := map[string]interface{}{
		"name":    tag,
		"type":    "url-test",
		"proxies": toStringSlice(outbound["outbounds"]),
	}

	if url, ok := outbound["url"].(string); ok && strings.TrimSpace(url) != "" {
		group["url"] = strings.TrimSpace(url)
	}
	if interval, ok := durationToSeconds(outbound["interval"]); ok && interval > 0 {
		group["interval"] = interval
	}
	if tolerance, ok := toInt(outbound["tolerance"]); ok && tolerance >= 0 {
		group["tolerance"] = tolerance
	}
	return group
}

func movePreferredMemberToFront(members []string, preferred string) []string {
	preferred = strings.TrimSpace(preferred)
	if preferred == "" || len(members) == 0 {
		return members
	}
...
func normalizeMihomoGroupMembers(raw interface{}, result *mihomoProxyConversionResult) []string {
	members := toStringSlice(raw)
	normalized := make([]string, 0, len(members))
	seen := map[string]struct{}{}
	for _, member := range members {
		target, ok := normalizeMihomoTarget(member, result)
		if !ok {
			continue
		}
...
func buildMihomoProxy(tag string, outType string, obMap map[string]interface{}) (map[string]interface{}, bool) {
		...
		if obfsOpts, ok := obMap["obfs_opts"].(map[string]interface{}); ok && obfsOpts != nil {
			...
			if mode != "" {
				proxy["obfs-opts"] = map[string]interface{}{
					"mode": mode,
					"host": normalizeMihomoSnellObfsHost(obfsOpts["host"]),
				}
			}
		}
	case "shadowsocks":
	...
	case "hysteria":
		util.NormalizeHysteriaOutboundOptionsMap(obMap)
		...
		if streamReceiveWindow, ok := toInt(obMap["stream_receive_window"]); ok && streamReceiveWindow > 0 {
			proxy["recv-window-conn"] = streamReceiveWindow
		}
		if connectionReceiveWindow, ok := toInt(obMap["connection_receive_window"]); ok && connectionReceiveWindow > 0 {
		...
		if disablePathMTUDiscovery, ok := toBool(obMap["disable_path_mtu_discovery"]); ok && disablePathMTUDiscovery {
			proxy["disable-mtu-discovery"] = true
		}
	...
	case "sudoku":
		...
		if httpmask := buildSudokuClashHTTPMask(obMap["httpmask"]); len(httpmask) > 0 {
	...
	if detourTag, ok := obMap["detour"].(string); ok && strings.TrimSpace(detourTag) != "" {
		proxy["dialer-proxy"] = strings.TrimSpace(detourTag)
	}
	if bindInterface, ok := obMap["bind_interface"].(string); ok && strings.TrimSpace(bindInterface) != "" {
		proxy["interface-name"] = strings.TrimSpace(bindInterface)
	}
	applyMihomoClashProxyCommonFields(proxy, obMap)
		...
		if outType != "trusttunnel" {
			if reality, ok := tlsMap["reality"].(map[string]interface{}); ok {
				if realityEnabled, ok := toBool(reality["enabled"]); ok && realityEnabled {
					...
					if pbk, ok := reality["public_key"].(string); ok && strings.TrimSpace(pbk) != "" {
						realityOpts["public-key"] = strings.TrimSpace(pbk)
					}
					if sid, ok := reality["short_id"].(string); ok && strings.TrimSpace(sid) != "" {
						realityOpts["short-id"] = strings.TrimSpace(sid)
					}
	...
	applyMihomoClashProxySMux(proxy, obMap)

	sanitizeUnsupportedMihomoClashProxyFields(proxy)
	return proxy, true
}

func normalizeMihomoSnellObfsHost(raw interface{}) string {
	host := strings.TrimSpace(firstString(raw))
	if host == "" {
		return "www.bing.com"
	}
	return host
...
func buildSudokuClashHTTPMask(raw interface{}) map[string]interface{} {
...
func buildMihomoShadowTLSSSProxy(ssOutbound map[string]interface{}, shadowTLSOutbound map[string]interface{}) (map[string]interface{}, bool) {
	...
	applyMihomoClashProxyCommonFields(proxy, ssOutbound)
	applyMihomoClashProxySMux(proxy, ssOutbound)
	proxy["plugin-opts"] = pluginOpts
	return proxy, true
}

func applyMihomoClashProxyCommonFields(proxy map[string]interface{}, obMap map[string]interface{}) {
...
func applyMihomoClashProxySMux(proxy map[string]interface{}, obMap map[string]interface{}) {
	if proxy == nil || obMap == nil {
		return
	}
...
func normalizeMihomoTarget(tag string, result *mihomoProxyConversionResult) (string, bool) {
	...
	if _, ok := result.DirectTags[normalized]; ok {
		return "DIRECT", true
	}
	if _, ok := result.SupportedTags[normalized]; ok {
...
func toInt(raw interface{}) (int, bool) {
	...
	fromFloat := func(value float64) (int, bool) {
		...
		// float64 cannot exactly represent MaxInt64. Values at 2^63 and above
		// are outside the positive int range on 64-bit builds.
...
func sanitizeUnsupportedMihomoClashProxyFields(proxy map[string]interface{}) {
	if proxy == nil {
		return
	}
	...
	case "tuic":
		...
		return
	case "hysteria2":
		if up, ok := toInt(proxy["up"]); ok && up <= 0 {
			delete(proxy, "up")
		}
		if down, ok := toInt(proxy["down"]); ok && down <= 0 {
			delete(proxy, "down")
		}
		return
	}
...
func durationToMicroseconds(raw interface{}) (int, bool) {
	if value, ok := toInt(raw); ok {
		return value, value > 0
	}
	value, ok := raw.(string)
	...
	if rawInt, err := strconv.Atoi(value); err == nil {
		return rawInt, rawInt > 0
	}
	if strings.HasSuffix(value, "us") {
		num := strings.TrimSpace(strings.TrimSuffix(value, "us"))
		us, err := strconv.Atoi(num)
		return us, err == nil && us > 0
	}
	if strings.HasSuffix(value, "ms") {
		num := strings.TrimSpace(strings.TrimSuffix(value, "ms"))
		ms, err := strconv.Atoi(num)
		return ms * 1000, err == nil && ms > 0
	}
	seconds := parseClashInterval(value)
	if seconds <= 0 {
		return 0, false
	}
	return seconds * 1000 * 1000, true
}
