...
// ClientRateLimitService manages per-port nftables bandwidth limiting rules.
//
...
func (s *ClientRateLimitService) InitOnStartup() {
	if runtime.GOOS != "linux" || !nftSupported() {
		return
	}
	if err := s.Reconcile(true); err != nil {
		logger.Warning("init client rate limit rules failed: ", err)
	}
}

func (s *ClientRateLimitService) EnsureRuleIntegrity() error {
	if runtime.GOOS != "linux" || !nftSupported() {
		return nil
	}
...
func (s *ClientRateLimitService) Reconcile(applyRules bool) error {
	db := database.GetDB()
	tx := db.Begin()
	if tx.Error != nil {
		return tx.Error
...
func (s *ClientRateLimitService) CleanupOnShutdown() {
	if runtime.GOOS == "linux" && nftSupported() {
		if err := deleteRulesByCommentPrefix(singboxLimitNftRuleComments.prefix); err != nil {
			logger.Warning("failed to cleanup client rate limit nft rules by prefix: ", err)
		}
	}

	db := database.GetDB()
	if err := db.Session(&gorm.Session{AllowGlobalUpdate: true}).
		Model(&model.ClientPortLimitState{}).
		Updates(map[string]interface{}{
			"in_handle":  0,
			"out_handle": 0,
			"updated_at": time.Now(),
		}).Error; err != nil {
		logger.Warning("failed to reset client rate limit handles after nft cleanup: ", err)
	}
}
...
func (s *ClientRateLimitService) reconcileStateOnly(tx *gorm.DB, desired map[int]int, desiredTags map[int]string) error {
	...
	existing := make(map[int]*model.ClientPortLimitState, len(states))
	for i := range states {
	...
	for _, port := range ports {
		...
		now := time.Now()
		next := model.ClientPortLimitState{
			Port:      port,
			Tag:       tag,
			LimitMbps: limit,
			InHandle:  0,
...
func (s *ClientRateLimitService) reconcileWithRules(tx *gorm.DB, desired map[int]int, desiredTags map[int]string) error {
	var states []model.ClientPortLimitState
	if err := tx.Find(&states).Error; err != nil {
		return err
	}
	...
	for _, state := range states {
		...
		if err := s.removeRulesFromState(&state); err != nil {
			logger.Warning("failed to remove obsolete client limit nft rules for port ", state.Port, ": ", err)
		}
		if err := tx.Delete(&state).Error; err != nil {
			return err
		}
	...
	for _, port := range ports {
		...
		validComments[singboxLimitNftRuleComments.in(portTag)] = struct{}{}
		validComments[singboxLimitNftRuleComments.out(portTag)] = struct{}{}

		if st, ok := existing[port]; ok {
			if st.LimitMbps != limit {
				if err := s.removeRulesFromState(st); err != nil {
					logger.Warning("failed to remove stale client limit rules for port ", port, ": ", err)
				}
...
func (s *ClientRateLimitService) ensureLimitRules(port int, limitMbps int) (int, int, error) {
	rateBytes := mbpsToBytesPerSecond(limitMbps)
	commentIn := singboxLimitNftRuleComments.in(strconv.Itoa(port))
	commentOut := singboxLimitNftRuleComments.out(strconv.Itoa(port))

	inHandle, inErr := addPortRateLimitRule(nftChainIn, port, "dport", rateBytes, commentIn)
	if inErr != nil {
		logger.Warning("failed to add client limit input rule for port ", port, ": ", inErr)
	}
	outHandle, outErr := addPortRateLimitRule(nftChainOut, port, "sport", rateBytes, commentOut)
	if outErr != nil {
		logger.Warning("failed to add client limit output rule for port ", port, ": ", outErr)
	}
...
func (s *ClientRateLimitService) removeRulesFromState(state *model.ClientPortLimitState) error {
	if state == nil {
		return nil
	}
	...
	} else if err := deleteRuleByComment(nftChainIn, singboxLimitNftRuleComments.in(portTag)); err != nil && firstErr == nil {
		firstErr = err
	}
	...
	} else if err := deleteRuleByComment(nftChainOut, singboxLimitNftRuleComments.out(portTag)); err != nil && firstErr == nil {
		firstErr = err
	}
...
func (s *ClientRateLimitService) tryRecoverHandles(tx *gorm.DB, state *model.ClientPortLimitState) error {
	if tx == nil || state == nil {
		return nil
	}
	changed := false
	...
	if handle := findHandleByComment(nftChainIn, singboxLimitNftRuleComments.in(portTag)); handle > 0 && handle != state.InHandle {
		state.InHandle = handle
		changed = true
	}
	if handle := findHandleByComment(nftChainOut, singboxLimitNftRuleComments.out(portTag)); handle > 0 && handle != state.OutHandle {
...
func ruleHandleExists(chain string, handle int) bool {
	if handle <= 0 {
		return false
	}
	_, err := getChainRuleBytesByHandle(chain, handle)
	return err == nil
}

func (s *ClientRateLimitService) collectDesiredPortLimits(tx *gorm.DB) (map[int]int, map[int]string, error) {
	...
	if err := tx.Model(&model.Inbound{}).Select("id, type, tag, options").Find(&inbounds).Error; err != nil {
		return nil, nil, err
	}
	...
	for _, inbound := range inbounds {
		baseInbound := model.Inbound{
			Id:      inbound.Id,
			Type:    inbound.Type,
			Tag:     inbound.Tag,
			Options: inbound.Options,
		}
		ports := expandPortRangesToPorts(collectInboundLimitRanges(&baseInbound))
		if len(ports) == 0 {
			continue
		}
...
func mbpsToBytesPerSecond(limitMbps int) int64 {
	if limitMbps <= 0 {
		return 0
	}
	// 1 Mbps = 1,000,000 bits/s = 125,000 bytes/s.
	return int64(limitMbps) * 125000
}

func sortedPortsFromMap(values map[int]int) []int {
	if len(values) == 0 {
		return []int{}
	}
